...
<body style="margin:0;padding:0;background:#F4ECDB;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;color:#15294C;">
  <div style="max-width:640px;margin:0 auto;padding:36px 24px 60px;">
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:24px;">
      <tr>
        <td>
          <div style="font-size:11px;letter-spacing:0.32em;text-transform:uppercase;color:#1F3F77;opacity:0.7;margin-bottom:6px;">New Cleaner Application</div>
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;background:#fff;border-radius:12px;overflow:hidden;border:1px solid rgba(31,63,119,0.12);box-shadow:0 4px 16px rgba(31,63,119,0.06);">
      ...
      <tr><td style="padding:18px 22px;font-size:14px;line-height:1.8;border-bottom:1px solid rgba(31,63,119,0.08);">
        ...
        ${c.email ? `<a href="mailto:${c.email}" style="color:#1F3F77;text-decoration:none;">${c.email}</a><br/>` : ''}
        ${p.city ? `Lives in: <strong>${p.city}</strong>` : ''}
      </td></tr>

      <tr><td style="padding:14px 22px;background:#15294C;color:#F4ECDB;font-size:11px;letter-spacing:0.28em;text-transform:uppercase;font-weight:600;">Background</td></tr>
      <tr><td style="padding:18px 22px;font-size:14px;line-height:1.8;border-bottom:1px solid rgba(31,63,119,0.08);">
        <strong>Experience:</strong> ${p.experience || '—'}<br/>
        <strong>Languages:</strong> ${p.language || '—'}
      </td></tr>

      <tr><td style="padding:14px 22px;background:#15294C;color:#F4ECDB;font-size:11px;letter-spacing:0.28em;text-transform:uppercase;font-weight:600;">Qualifications</td></tr>
      <tr><td style="padding:18px 22px;font-size:14px;line-height:1.8;border-bottom:1px solid rgba(31,63,119,0.08);">
        <strong>Own transportation:</strong> <span style="color:${p.ownTransport ? '#1F3F77' : '#B83A3A'};">${yesNo(p.ownTransport)}</span><br/>
        <strong>US authorized:</strong> <span style="color:${p.usAuthorized ? '#1F3F77' : '#B83A3A'};">${yesNo(p.usAuthorized)}</span><br/>
